...
def get_value_by_mode(mem, pm, rba, mode):
    ...
    try:
        ...
        if pos >= len(mem): return 0

def store_by_mode(mem, pm, rba, mode, value):
    if mode == 1: # Immediate mode
...
def add(mem, pos, rba, md1, md2, md3, dbg):
    nw_pos = pos + 4
    [pm1, pm2, pm3] = mem[pos+1:nw_pos]
    value1 = get_value_by_mode(mem, pm1, rba, md1)
    value2 = get_value_by_mode(mem, pm2, rba, md2)
    result = value1 + value2
    mem, store = store_by_mode(mem, pm3, rba, md3, result)
    if dbg: print_instr(pos, [pm1, pm2, pm3], f'ADD {value1} + {value2}.', f'Store {result} at {store}')
    return mem, nw_pos
    
def multiply(mem, pos, rba, md1, md2, md3, dbg):
    nw_pos = pos + 4
    [pm1, pm2, pm3] = mem[pos+1:nw_pos]
    value1 = get_value_by_mode(mem, pm1, rba, md1)
    value2 = get_value_by_mode(mem, pm2, rba, md2)
...
def store_input(mem, pos, rba, md1, dbg, inp):
    nw_pos = pos + 2
    [pm1]  = mem[pos+1:nw_pos]
    value  = inp.pop(0)
    mem, store = store_by_mode(mem, pm1, rba, md1, value)
    if dbg: print_instr(pos, [pm1], f'STO Input {value}.', f'Store {value} at {store}')
    return mem, nw_pos, inp

def output(mem, pos, rba, md1, dbg, out):
    nw_pos = pos + 2
    [pm1]  = mem[pos+1:nw_pos]
    value  = get_value_by_mode(mem, pm1, rba, md1)
    out.append(value)
    if dbg: print_instr(pos, [pm1], f'OUT Mem {value}.', f'Store {value} at output {len(out)-1}')
    return mem, nw_pos, out

def jump_if_true(mem, pos, rba, md1, md2, dbg):
    nw_pos  = pos + 3
    [pm1, pm2] = mem[pos+1:nw_pos]
    value   = get_value_by_mode(mem, pm1, rba, md1)
    jump_to = get_value_by_mode(mem, pm2, rba, md2)
    if value != 0: 
        nw_pos = jump_to
        msg = f'Jump to {jump_to}'
    else:
        msg = 'No jump'
    if dbg: print_instr(pos, [pm1, pm2], f'JTR Compare {value} != 0.', msg)
    return mem, nw_pos

def jump_if_false(mem, pos, rba, md1, md2, dbg):
    nw_pos  = pos + 3
    [pm1, pm2] = mem[pos+1:nw_pos]
    value   = get_value_by_mode(mem, pm1, rba, md1)
    jump_to = get_value_by_mode(mem, pm2, rba, md2)
    if value == 0: 
        ...
        msg = 'No jump'
    if dbg: print_instr(pos, [pm1, pm2], f'JFA Compare {value} == 0.', msg)
    return mem, nw_pos

def less_than(mem, pos, rba, md1, md2, md3, dbg):
    nw_pos = pos + 4
    [pm1, pm2, pm3] = mem[pos+1:nw_pos]
    value1 = get_value_by_mode(mem, pm1, rba, md1)
    value2 = get_value_by_mode(mem, pm2, rba, md2)
...
def equals(mem, pos, rba, md1, md2, md3, dbg):
    nw_pos = pos + 4
    [pm1, pm2, pm3] = mem[pos+1:nw_pos]
    value1 = get_value_by_mode(mem, pm1, rba, md1)
    value2 = get_value_by_mode(mem, pm2, rba, md2)
...
def adjust_rba(mem, pos, rba, md1, dbg):
    nw_pos = pos + 2
    [pm1]  = mem[pos+1:nw_pos]
    value  = get_value_by_mode(mem, pm1, rba, md1)
    nw_rba = rba + value
    if dbg: print_instr(pos, [pm1], f'ARB {rba} + {value}.', f'Set relative base to {nw_rba}')
...
def run(program: Program, debug=True):
    ...
    while True:
        ...
        elif opc == 1: mem, pos = add(mem, pos, rba, md1, md2, md3, dbg)
        elif opc == 2: mem, pos = multiply(mem, pos, rba, md1, md2, md3, dbg)
        elif opc == 3: mem, pos, inp = store_input(mem, pos, rba, md1, dbg, inp) 
        elif opc == 4: mem, pos, out = output(mem, pos, rba, md1, dbg, out)   
        elif opc == 5: mem, pos = jump_if_true(mem, pos, rba, md1, md2, dbg) 
        elif opc == 6: mem, pos = jump_if_false(mem, pos, rba, md1, md2, dbg) 
        elif opc == 7: mem, pos = less_than(mem, pos, rba, md1, md2, md3, dbg) 
        elif opc == 8: mem, pos = equals(mem, pos, rba, md1, md2, md3, dbg) 
        elif opc == 9: mem, pos, rba = adjust_rba(mem, pos, rba, md1, dbg) 
